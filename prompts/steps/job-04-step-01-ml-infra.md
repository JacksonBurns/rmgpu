...
(the model team's own code, now in this repo), and build the shared load
path. NO synthetic test models: the checkpoints are real and in the tree,
so the tests run them directly.
...
predictions for a fixed set of molecules/reactions. The reference
baseline is generated by models/predict.py ITSELF (the model team's code,
run directly), so the round-trip test in steps 2/3 checks rmgpu.ml.base
against the model team's own output - not against base.py's own output
...
## Reference to read (this step's budget)
  ...
  models/chemeleon_thermo_662946.ckpt + models/chemprop_kinetics_662946.ckpt
...
- scripts/record_reference_predictions.py (re-runnable, idempotent): runs
  the two vendored checkpoints through models/predict.py's own pattern
  (the model team's code - NOT rmgpu.ml.base, to keep the baseline
  non-circular) on a FIXED set: molecules CC, CCC, C[CH]CC + the two
  reaction SMILES from models/predict.py's __main__; writes the raw
  predictions (the model's log10-space outputs, full precision) to
  gates/baselines/job04/reference_predictions.json. Step 2/3 and the gate
  assert rmgpu.ml.base reproduces these values.
...
- reports/job-04-step-01.md checkpoint inventory: for each vendored
  checkpoint in models/ (chemeleon_thermo_662946.ckpt,
  chemprop_kinetics_662946.ckpt): path, loadable (verified in this env),
  ...
  the fixed-set numbers from the baseline above (commit
  gates/baselines/job04/reference_predictions.json) so step 2/3 and the
  gate can assert the estimators reproduce them.
- tests/test_ml_base.py: BOTH real vendored checkpoints load via
  rmgpu.ml.base; predict the step-1 fixed set (3 molecules / 2 reactions)
  through base.predict_raw and assert the shapes + that the values match
  the recorded reference_predictions.json (tol 1e-4); determinism (two
  runs -> same values). If a checkpoint does not load, record the exact
  failure - but they should, it was verified before this job.

## Checks (must run and pass before you claim done)

  /home/jackson/miniforge3/envs/rmgpu/bin/python
    scripts/record_reference_predictions.py -> baseline written (or
    confirmed unchanged)
  pytest tests/test_ml_base.py -q -> all pass (includes both real
    checkpoints loading via rmgpu.ml.base and matching the baseline)
  the reference-prediction numbers are recorded in the report/baselines

## Pitfalls

- No synthetic test models (dropped 2026-08-28): the checkpoints are real
  and in the tree, so the tests run them directly. The reference baseline
  must come from models/predict.py's OWN pattern (the model team's code),
  not from rmgpu.ml.base - asserting base.py reproduces base.py's own
