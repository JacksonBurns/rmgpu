...
- gates/gate_04.py: implements the job definition:
    ...
    accuracy; the step-01 baseline is generated by models/predict.py itself,
    so the round-trip is checked against the model team's own code, not
    against base.py's own output).
