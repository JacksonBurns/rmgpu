...
2. STATUS.md: set your step row to `done` and append a session-log entry: `### <date> - job-09/step-02 / built: ... / checks: ... (GREEN|RED + one line) / commits: <hashes> / next: <the next step id>`. Update the top-level NEXT pointer to the next step's file.
